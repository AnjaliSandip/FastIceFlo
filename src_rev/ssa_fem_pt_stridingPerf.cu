...
/*CUDA Code*/
__global__ void PT1(double* vx, double* vy, double* alpha, double* alpha_0, double* alpha_1, double* alpha_2, double* beta_0, double* beta_1, double* beta_2, int* index_0, int* index_1, int* index_2,  double* kvx, double* kvy, double* etan,  double* Helem, double* areas, bool* isice, double* Eta_nbe, double* rheology_B, double n_glen, double eta_0, double rele,int nbe){     
  ...
  //      for(int i=0; i<3; i++){
     ...
     //   }
         ...
         double dvxdx = alpha[ix + nbe*0] + alpha[ix +nbe*1] + alpha[ix + nbe*2];   //row wise    
     ...
     //  double dvxdx =  vx[index_0[ix]]* alpha_0[ix] + vx[index_1[ix]]*  alpha_1[ix] + vx[index_2[ix]]*  alpha_2[ix];
     //   double dvxdy =  vx[index_0[ix]]* beta_0[ix]  + vx[index_1[ix]]*  beta_1[ix]  + vx[index_2[ix]]*  beta_2[ix];
      //  double dvydx =  vy[index_0[ix]]* alpha_0[ix] + vy[index_1[ix]]*  alpha_1[ix] + vy[index_2[ix]]*  alpha_2[ix];
      //  double dvydy =  vy[index_0[ix]]* beta_0[ix]  + vy[index_1[ix]]*  beta_1[ix]  + vy[index_2[ix]]*  beta_2[ix];
 ...
 //         for (int i = 0; i < 3; i++){
   ...
   //  }
kvx[ix*3+0] = 2 * Helem[ix] * etan[ix] * (2 * eps_xx + eps_yy) * alpha_0[ix] * areas[ix] + 2 * Helem[ix] * etan[ix] * eps_xy *  beta_0[ix] * areas[ix];
kvx[ix*3+1] = 2 * Helem[ix] * etan[ix] * (2 * eps_xx + eps_yy) * alpha_1[ix] * areas[ix] + 2 * Helem[ix] * etan[ix] * eps_xy *  beta_1[ix] * areas[ix];
kvx[ix*3+2] = 2 * Helem[ix] * etan[ix] * (2 * eps_xx + eps_yy) * alpha_2[ix] * areas[ix] + 2 * Helem[ix] * etan[ix] * eps_xy *  beta_2[ix] * areas[ix];


kvy[ix*3+0] = 2 * Helem[ix] * etan[ix] * (2 * eps_yy + eps_xx) *  beta_0[ix] * areas[ix] + 2 * Helem[ix] * etan[ix] * eps_xy * alpha_0[ix] * areas[ix];
kvy[ix*3+1] = 2 * Helem[ix] * etan[ix] * (2 * eps_yy + eps_xx) *  beta_1[ix] * areas[ix] + 2 * Helem[ix] * etan[ix] * eps_xy * alpha_1[ix] * areas[ix];  
kvy[ix*3+2] = 2 * Helem[ix] * etan[ix] * (2 * eps_yy + eps_xx) *  beta_2[ix] * areas[ix] + 2 * Helem[ix] * etan[ix] * eps_xy * alpha_2[ix] * areas[ix];  
        }//isice loop
...
int main(){
    ...
    const char* inputfile  = "./PIG3e4.bin";
    ...
    double* beta_0   = new double[nbe];
    double* beta_1   = new double[nbe];
    double* beta_2   = new double[nbe];

    int* index_0   = new int[nbe];
    int* index_1   = new int[nbe];
    int* index_2   = new int[nbe];
        ...
        for(int i=0;i<nbe;i++){
           ...
           beta_1[i] = beta[i*3+1];
           beta_2[i] = beta[i*3+2];
           index_0[i] = index[i*3+0]-1;
           index_1[i] = index[i*3+1]-1;
           index_2[i] = index[i*3+2]-1;
        }    
 ...
 //   cudaMemcpy(d_beta, beta, nbe*3*sizeof(double), cudaMemcpyHostToDevice);
    ...
    double *d_beta_0;
    cudaMalloc(&d_beta_0, nbe*sizeof(double));
    cudaMemcpy(d_beta_0, beta_0, nbe*sizeof(double), cudaMemcpyHostToDevice);

    double *d_beta_1;
    cudaMalloc(&d_beta_1, nbe*sizeof(double));
    cudaMemcpy(d_beta_1, beta_1, nbe*sizeof(double), cudaMemcpyHostToDevice);

    double *d_beta_2;
    cudaMalloc(&d_beta_2, nbe*sizeof(double));
    cudaMemcpy(d_beta_2, beta_2, nbe*sizeof(double), cudaMemcpyHostToDevice);

    int *d_index_0;
    cudaMalloc(&d_index_0, nbe*sizeof(int));
    cudaMemcpy(d_index_0, index_0, nbe*sizeof(int), cudaMemcpyHostToDevice);
    
    int *d_index_1;
    cudaMalloc(&d_index_1, nbe*sizeof(int));
    cudaMemcpy(d_index_1, index_1, nbe*sizeof(int), cudaMemcpyHostToDevice);

    int *d_index_2;
    cudaMalloc(&d_index_2, nbe*sizeof(int));
    cudaMemcpy(d_index_2, index_2, nbe*sizeof(int), cudaMemcpyHostToDevice);
    ...
    for(iter=1;iter<=niter;iter++){
        ...
        PT1<<<gride, blocke>>>(d_vx, d_vy,d_alpha,  d_alpha_0, d_alpha_1, d_alpha_2, d_beta_0, d_beta_1, d_beta_2, d_index_0, d_index_1, d_index_2, kvx,  kvy, d_etan, d_Helem, d_areas, d_isice, Eta_nbe, d_rheology_B, n_glen, eta_0, rele, nbe);        cudaDeviceSynchronize();
