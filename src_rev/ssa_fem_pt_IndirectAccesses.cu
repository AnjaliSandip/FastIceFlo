...
__global__ void PT1(double* vx, double* vy, double* Vx, double* Vy, double* alpha, double* beta, int* index,  double* kvx, double* kvy, double* etan,  double* Helem, double* areas, bool* isice, double* Eta_nbe, double* rheology_B, double n_glen, double eta_0, double rele,int nbe){ 
    ...
    for(int ix = blockIdx.x * blockDim.x + threadIdx.x; ix<nbe; ix += blockDim.x * gridDim.x){ 
        ...
        for(int i=0; i<3; i++){
            ...
            Localvx[i] = Vx[ix*3+i];
            Localvy[i] = Vy[ix*3+i];
//            Vx[ix*3+i]  =  vx[index[ix*3+i]-1];
//            Vy[ix*3+i]  =  vy[index[ix*3+i]-1];
...
__global__ void PT4(double* vx, double* vy, double* Vx, double* Vy, int* index,  int nbe){
   ...
   for(int ix = blockIdx.x * blockDim.x + threadIdx.x; ix<nbe; ix += blockDim.x * gridDim.x){
    ...
    for(int i=0; i<3; i++){
            localindex[i] = index[ix*3+i]-1;  
        }
		
     for(int j=0;j<3;j++) {
	Vx[ix*3+j] = vx[localindex[j]];
	Vy[ix*3+j] = vy[localindex[j]];
        } 
    } 
...
int main(){
    ...
    const char* inputfile  = "./PIG7e4.bin";
    ...
    /*Constants*/
    double n_glen     = 3.;
    double damp       = 0.99; //0.96 for JKS2e4, 0.981 for PIG3e4
    ...
    double relaxation = 0.9; //0.7 for JKS2e4, 0.967 for PIG3e4
...
//        cudaStreamSynchronize(stream2);
      ...
      PT4<<<gridv, blockv>>>(d_vx, d_vy, d_Vx, d_Vy, d_index, nbe); cudaDeviceSynchronize();
