...
__global__ void elem2node_GPU (double* eta_nbv, double* eta_nbv_gpu, double* etan, int* index, double* areas, double* weights, int nbe, int nbv){
...
cudaMemcpy(d_spcvy, spcvy, nbv*sizeof(double), cudaMemcpyHostToDevice);
    ...
    for(iter=1;iter<=niter;iter++){
        ...
        /*Get current viscosity on nodes (Needed for time stepping), eta_nbv  - GPU KERNEL 3 */
        elem2node_GPU <<<gridSize,blockSize>>> (d_eta_nbv, eta_nbv_gpu, d_etan, d_index, d_areas, d_weights, nbe, nbv); cudaDeviceSynchronize();
