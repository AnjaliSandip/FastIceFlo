...
__global__ void derive_xy_elem_GPU(double* dfdx_e,double* dfdy_e, double* f, double* alpha,double* beta, int* index,int nbe) {
...
__global__ void elem2node_GPU (double* eta_nbv_gpu, double* etan, int* index, double* areas, double* weights, int nbe, int nbv){
...
for (int i=0;i<nbe;i++) {
	...
	eta_nbv_gpu[ix*3+i] = etan[ix+i]*areas[ix+i];
...
__global__ void KV (double* KVxx, double* KVyy, double* etan, double* dvxdx, double* dvydy, double* dvxdy, double* dvydx, double* Helem, double* alpha, double* beta, double* areas, int *index, int nbe) {
...
double *d_dvxdx;
cudaMalloc(&d_dvxdx, nbe*sizeof(double));
cudaMemcpy(d_dvxdx, dvxdx, nbe*sizeof(double), cudaMemcpyHostToDevice);
...
//dvxdy, nbe//
double *d_dvxdy;
cudaMalloc(&d_dvxdy, nbe*sizeof(double));
cudaMemcpy(d_dvxdy, dvxdy, nbe*sizeof(double), cudaMemcpyHostToDevice);  
      
   

//dvydx, nbe//
double *d_dvydx;
cudaMalloc(&d_dvydx, nbe*sizeof(double));
cudaMemcpy(d_dvydx, dvydx, nbe*sizeof(double), cudaMemcpyHostToDevice);  

//dvydy, nbe//
double *d_dvydy;
cudaMalloc(&d_dvydy, nbe*sizeof(double));
cudaMemcpy(d_dvydy, dvydy, nbe*sizeof(double), cudaMemcpyHostToDevice);  
...
//KVx, nbv//
//double *d_KVx;
//cudaMalloc(&d_KVx, nbv*sizeof(double));
//cudaMemcpy(d_KVx, KVx, nbv*sizeof(double), cudaMemcpyHostToDevice);


//KVy, nbe//
//double *d_KVy;
//cudaMalloc(&d_KVy, nbv*sizeof(double));
//cudaMemcpy(d_KVy, KVy, nbv*sizeof(double), cudaMemcpyHostToDevice);


//KVxx, nbe//
double *d_KVxx;
cudaMalloc(&d_KVxx, nbe*sizeof(double));
cudaMemcpy(d_KVxx, KVxx, nbe*sizeof(double), cudaMemcpyHostToDevice);


//KVyy, nbe//
double *d_KVyy;
cudaMalloc(&d_KVyy, nbe*sizeof(double));
cudaMemcpy(d_KVyy, KVyy, nbe*sizeof(double), cudaMemcpyHostToDevice);
...
//eta_nbv_gpu, nbe//
double *d_eta_nbv_gpu;
cudaMalloc(&d_eta_nbv_gpu, nbe*sizeof(double));
cudaMemcpy(d_eta_nbv_gpu, eta_nbv_gpu, nbe*sizeof(double), cudaMemcpyHostToDevice);
...
derive_xy_elem_GPU <<<gridSize,blockSize>>> (d_dvxdx, d_dvxdy, d_vx, d_alpha, d_beta, d_index, nbe); cudaDeviceSynchronize();
derive_xy_elem_GPU <<<gridSize,blockSize>>> (d_dvydx, d_dvydy, d_vy, d_alpha, d_beta, d_index, nbe); cudaDeviceSynchronize();
...
KV <<<gridSize,blockSize>>> (d_KVxx, d_KVyy, d_etan, d_dvxdx, d_dvydy, d_dvxdy, d_dvydx, d_Helem, d_alpha, d_beta, d_areas, d_index, nbe); cudaDeviceSynchronize(); 

cudaMemcpy(KVxx, d_KVxx, nbe*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(KVyy, d_KVyy, nbe*sizeof(double), cudaMemcpyDeviceToHost );      

  for(int i=0;i<nbe;i+=3){
              KVx[index[i*3+0]-1] += KVxx[i];
              KVx[index[i*3+1]-1] += KVxx[i+1];
              KVx[index[i*3+2]-1] += KVxx[i+2];
              KVy[index[i*3+0]-1] += KVyy[i];
              KVy[index[i*3+1]-1] += KVyy[i+1];
              KVy[index[i*3+2]-1] += KVyy[i+2];
...
elem2node_GPU <<<gridSize,blockSize>>> (d_eta_nbv_gpu, d_etan,d_index,d_areas, d_weights, nbe, nbv); cudaDeviceSynchronize();


cudaMemcpy(eta_nbv_gpu, d_eta_nbv_gpu, nbe*sizeof(double), cudaMemcpyDeviceToHost );



 for(int i=0;i<nbe;i++){
            eta_nbv[index[i*3+0]-1] += eta_nbv_gpu[i];
            eta_nbv[index[i*3+1]-1] += eta_nbv_gpu[i+1];
            eta_nbv[index[i*3+2]-1] += eta_nbv_gpu[i+2];
...
cudaMemcpy(index, d_index, nbe*3*sizeof(int), cudaMemcpyDeviceToHost );
cudaMemcpy(dvxdx, d_dvxdx, nbe*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(vx, d_vx, nbv*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(alpha, d_alpha, nbe*3*sizeof(double), cudaMemcpyDeviceToHost );   
cudaMemcpy(beta, d_beta, nbe*3*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(dvxdy, d_dvxdy, nbe*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(dvydx, d_dvydx, nbe*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(dvydy, d_dvydy, nbe*sizeof(double), cudaMemcpyDeviceToHost );
...
cudaMemcpy(Helem, d_Helem, nbe*sizeof(double), cudaMemcpyDeviceToHost );
   
cudaMemcpy(etan, d_etan, nbe*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(areas, d_areas, nbe*sizeof(double), cudaMemcpyDeviceToHost );
//cudaMemcpy(eta_nbv, d_eta_nbv, nbv*sizeof(double), cudaMemcpyDeviceToHost );
cudaMemcpy(weights, d_weights, nbv*sizeof(double), cudaMemcpyDeviceToHost );


 
 for(int i=0;i<nbv;i++){
       std::cout<<"KVx "<<KVx[i] <<std::endl;

    }


 for(int i=0;i<nbv;i++){
      std::cout<<"KVy "<<KVy[i] <<std::endl;
   ...
   /*Cleanup intermediary vectors*/
    ...
    WriteData(fid,KVx,nbv,1,"KVx");
    WriteData(fid,KVy,nbv,1,"KVy");
